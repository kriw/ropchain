...
void parseArgs(int argc, char **argv) {
    static struct option ops[] = {
        {"rax", 1, NULL, 'A'},      {"eax", 1, NULL, 'A'},
        {"rbx", 1, NULL, 'B'},      {"ebx", 1, NULL, 'B'},
        {"rcx", 1, NULL, 'C'},      {"ecx", 1, NULL, 'C'},
        {"rdx", 1, NULL, 'D'},      {"edx", 1, NULL, 'D'},
        {"rdi", 1, NULL, 'E'},      {"edi", 1, NULL, 'E'},
        {"rsi", 1, NULL, 'F'},      {"esi", 1, NULL, 'F'},
        {"r8", 1, NULL, 'G'},       {"r9", 1, NULL, 'H'},
        {"r10", 1, NULL, 'I'},      {"r11", 1, NULL, 'J'},
        {"r12", 1, NULL, 'K'},      {"r13", 1, NULL, 'L'},
        {"r14", 1, NULL, 'M'},      {"r15", 1, NULL, 'N'},
        {"fastcall", 1, NULL, 'X'}, {"cdecl", 1, NULL, 'Y'},
    };
    int ops_index;
    while (true) {
    ...
    optind = 0;
    while ((c = getopt_long(argc, argv, "a:b:df:g:hi:", ops, &ops_index)) !=
           EOF) {
        switch (c) {
        case 'b':
        ...
        case 'X':
            ...
            // solver = [&funcAddr](const std::map<RegType::Reg, uint64_t>&
            // dests, const std::string& file,
            //         uint64_t base, const std::set<char>& avoids) {
            //     return Builder::fastCall(funcAddr, std::vector<uint64_t>(),
            //     dests, file, base, avoids);
            // };
            break;
        case 'Y':
            // cdecl
            // TODO args
            // const uint64_t funcAddr = fromStr(optarg);
            // solver = [&funcAddr](auto dests, auto file, auto base, auto
            // avoids) {
            //     return Builder::cdeclCall(funcAddr, std::vector<uint64_t>(),
            //     dests, file, base, avoids);
            // };
            break;
        }
