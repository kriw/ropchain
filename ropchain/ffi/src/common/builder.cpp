...
OptROP Builder::cdecl(uint64_t funcAddr, const std::vector<uint64_t> &args,
    ...
    const auto popN =
        Util::findByUseStack(gadgets, remainNum * Config::Arch::word());
    if (!popN.has_value()) {
        return {};
    }
    rop += ROPChain(popN.value());
    return std::accumulate(
        args.begin(), args.end(), rop,
...
OptROP Builder::fastcall(uint64_t funcAddr, const std::vector<uint64_t> &args,
    ...
    if (Config::getArch() == Config::Arch::X86) {
        argRegs = std::vector<RegType::Reg>({RegType::ecx, RegType::edx});
    } else if (Config::getArch() == Config::Arch::AMD64) {
        argRegs =
            std::vector<RegType::Reg>({RegType::rdi, RegType::rsi, RegType::rdx,
    ...
    const size_t remainNum =
        argRegs.size() < args.size() ? args.size() - argRegs.size() : 0;
    ...
    return rop.value() + ROPChain(funcAddr) + ROPChain(popN.value()) +
           remainArgs;
}
