...
OptROP Solver::_solve(const std::map<RegType::Reg, uint64_t> &dests,
                      const Gadgets &gadgets, uint64_t base, Cond &cond,
                      Proc &proc, const std::set<char> &avoids) {
    ...
    for (const auto reg : allBits) {
        const auto tmp = findROPChain(reg, dests.at(reg), gadgets, RegSet(reg),
                                      cond, proc, avoids);
        if (tmp.has_value()) {
    ...
    if (remains.none()) {
        auto rop = std::accumulate(
            ropChains.begin(), ropChains.end(), ROPChain(),
    ...
    do {
        ...
        for (RegType::Reg reg : bits) {
            const auto tmp = findROPChain(reg, dests.at(reg), gadgets, aval,
                                          cond, proc, avoids);
            if (!tmp.has_value()) {
                isDone = false;
                break;
...
OptROP solveByInc(const RegType::Reg reg, const uint64_t dest,
...
OptROP solveByPopInc(const RegType::Reg reg, const uint64_t dest,
                     const Gadgets &gadgets, const std::set<char> &avoids,
                     const std::vector<uint8_t> &chars, RegSet aval) {
    uint64_t tmpDest = 0;
    ...
    for (size_t i = 0; i < word; i++) {
        ...
        std::copy_if(chars.begin(), chars.end(), std::back_inserter(filtered),
            ...
            *std::max_element(filtered.begin(), filtered.end()) == byte) {
            ...
            while (!p(tmpDest) && tmpDest > 0) {
                ...
                if (tmpDest <= 0) {
                    ...
                    const uint8_t a =
                        *std::max_element(chars.begin(), chars.end());
                    tmpDest = (tmpDest << (bits - i * 8));
...
// use xor r1, r2
OptROP solveByXor(const RegType::Reg reg, const uint64_t dest,
                  const Gadgets &gadgets, const std::set<char> &avoids,
                  const std::vector<uint8_t> &chars, RegSet aval) {
    OptROP rop = {};
    ...
    const auto ropLeft =
        Solver::findROPChain(reg, left, gadgets, aval, {}, {}, avoids);
    ...
    for (const auto r : allBits) {
        if (const auto _xor = Xor::find(reg, r, gadgets, aval)) {
            ...
            if (ropLeft.has_value() && ropRight.has_value()) {
                const OptROP _rop =
                    ropLeft.value() + ropRight.value() + _xor.value();
...
OptROP Solver::solveAvoidChars(const std::map<RegType::Reg, uint64_t> &dests,
                               const Gadgets &gadgets, const uint64_t base,
    ...
    Proc proc = [](const RegType::Reg reg, const uint64_t dest,
        ...
        rop = Util::optMin(rop,
                           solveByXor(reg, dest, gadgets, avoids, chars, aval));
        rop = Util::optMin(
            rop, solveByPopInc(reg, dest, gadgets, avoids, chars, aval));
        rop = Util::optMin(rop, solveByInc(reg, dest, gadgets, aval));
        return rop;
    };
    auto _gadgets = Gadgets();
    std::copy_if(gadgets.begin(), gadgets.end(), std::back_inserter(_gadgets),
                 [&cond, &base](auto &g) {
                     return !g.isUseless && cond(g.addr + base);
                 });
    return _solve(dests, Util::uniqGadgets(_gadgets), base, cond, proc, avoids);
}

OptROP Solver::solveWithGadgets(const std::map<RegType::Reg, uint64_t> &dests,
                                const Gadgets &gadgets, const uint64_t base,
                                const std::set<char> &avoids) {
    return solveAvoidChars(dests, gadgets, base, avoids);
}

OptROP Solver::solveWithFile(const std::map<RegType::Reg, uint64_t> &dests,
...
OptROP Solver::findROPChain(const RegType::Reg reg, const uint64_t dest,
                            const Gadgets &gadgets, RegSet aval,
                            std::optional<Cond> cond, std::optional<Proc> proc,
                            const std::set<char> &avoids) {
