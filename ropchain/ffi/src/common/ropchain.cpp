...
ROPChain::ROPChain() : baseAddr(0) {}

ROPChain::ROPChain(const ROPElem elem) : baseAddr(0) {
    ROPElems es = std::visit(
        [](const auto &e) {
            ...
            if constexpr (std::is_same_v<T, Gadget>) {
                ...
                auto s =
                    std::string(e.first.useStack - Config::Arch::word(), 'A');
                return ROPElems({e, s});
            }
            return ROPElems({e});
        },
...
void ROPChain::setBaseAddr(const uint64_t addr) { baseAddr = addr; }
        ...
        payload += std::visit(
            [&](auto &&e) {
                ...
                if constexpr (std::is_same_v<T, Gadget>) {
                    ...
                    return Util::pack(e.first.addr + baseAddr) +
                           Util::pack(e.second);
                } else if constexpr (std::is_same_v<T, std::string>) {
                    return e;
                } else if constexpr (std::is_same_v<T, uint64_t>) {
...
ROPElems ROPChain::getElems() const { return elems; }

// unit: byte
size_t ROPChain::length() const { return elems.size() * Config::Arch::word(); }
