...
Insn::Insn(Mnem _mnem, std::vector<Operand> _ops)
    : hash(calcHash(_mnem, _ops)), mnem(std::move(_mnem)),
      ops(std::move(_ops)){};
...
Insn &Insn::operator=(const Insn &insn) {
    if (this != &insn) {
        ...
        _ops->clear();
        std::copy(insn.ops.begin(), insn.ops.end(), std::back_inserter(*_ops));
    }
    return *this;
}
...
bool Insn::operator==(const Insn &insn) const {
    ...
    const auto check = [](auto &&a, auto &&b) {
        ...
        if constexpr (std::is_same_v<T, MemOp> && std::is_same_v<U, MemOp>) {
...
bool Insn::operator!=(const Insn &insn) const { return !(*this == insn); }
...
size_t Insn::calcHash(const Mnem &mnem, const std::vector<Operand> &ops) {
    return (std::hash<std::string>{}(mnem) << 1) ^
           std::hash<std::vector<Operand>>{}(ops);
}
