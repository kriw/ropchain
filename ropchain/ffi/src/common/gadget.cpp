...
// XXX
Gadget::Gadget()
    : insns(std::vector<Insn>()), addr(0), useStack(0), changedRegs(RegSet()),
      hash(0), isUseless(calcIsUseless(insns)) {}

Gadget::Gadget(const uint64_t _addr, const std::vector<Insn> &_insns)
    : insns(_insns), addr(_addr), useStack(Util::calcUseStack(_insns)),
      changedRegs(Util::listChangedRegs(insns)), hash(calcHash(_insns)),
