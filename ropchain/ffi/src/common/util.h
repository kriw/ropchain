...
#endif
#ifdef _DEBUG
template <typename T> void ERR(T &&x) {
    std::cerr << std::forward<T>(x) << std::endl;
}

template <typename T, typename... S> void ERR(T &&x, S &&... xs) {
    std::cerr << std::forward<T>(x);
    ERR(std::forward<S>(xs)...);
}
#else
template <typename T, typename... S> void ERR(T &&x, S &&... xs) { UNUSED(x); };
#endif

namespace Util {
RegSet allRegs();
RegSet map2Regs(const std::map<RegType::Reg, uint64_t> &m);
std::optional<Gadget> parseGadgetString(const uint64_t addr,
                                        const std::string &gadgetStr);
...
template <typename T> T optMin(T t1, T t2) {
    if (!t1.has_value()) {
        return t2;
    }
    if (!t2.has_value()) {
...
OptGadget find(const Gadgets &gadgets, const RegSet &avl, const Mnem &mnem,
               const std::optional<Operand> op1,
