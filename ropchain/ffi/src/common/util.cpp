...
std::optional<Gadget> Util::parseGadgetString(const uint64_t addr,
...
std::string Util::join(const std::vector<std::string> &s,
...
inline void rtrim(std::string &s, const std::string &delims) {
    s.erase(std::find_if(
                s.rbegin(), s.rend(),
                [&](int ch) { return delims.find(ch) == std::string::npos; })
                .base(),
            s.end());
}
...
void Util::resetMemo() { findMemo.clear(); }
