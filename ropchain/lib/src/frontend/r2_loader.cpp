...
std::string Frontend::test(const char *file) {
	...
	if (r2) {
		auto ret = r2p_cmd (r2, "\"/R/j pop rax;ret\"");
		r2p_close (r2);
		return ret;
	}
	return "";
...
Gadgets Frontend::fromCmd(R2Pipe *r2, const std::string& cmd) {
    auto gadgets = Gadgets();
    // auto outputJson = json::parse("[" + std::string(r2p_cmd(r2, cmd.c_str())) + "]");
...
std::optional<Gadgets> Frontend::fromR2(const std::string& fileName) {
    ...
    //TODO more gadgets
    return fromCmd(r2, "\"/R/j pop r[abcd]x;ret$\"");
}
